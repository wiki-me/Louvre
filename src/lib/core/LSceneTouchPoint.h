...
public:
     ...
     * @brief Touched views
     *
     * List of views touched by this touch point. Updated each time a touch down event is handled.
     */
    const std::list<LView*> &views() const;
    ...
    ~LSceneTouchPoint();
    std::list<LSceneTouchPoint*>::iterator destroy();
};
